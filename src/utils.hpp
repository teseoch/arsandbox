...
static void generateDepthFrame(Depth &depth, double t)
{
	...
	for (int y = 0; y < H; y++)
	{
		for (int x = 0; x < W; x++)
		{
			...
			float h = a1 * gauss(fx, fy, cx1, cy1, 0.12f) + a2 * gauss(fx, fy, cx2, cy2, 0.10f) + a3 * gauss(fx, fy, cx3, cy3, 0.08f);

			// a little ripple to mimic sand micro-variation
			float rip = 8.0f * std::sin(12.0f * fx + 9.0f * fy + (float)t) * std::sin(10.0f * fy + (float)t * 0.7f);
